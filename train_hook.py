...
print('-------------------------------------------------')
print("input 결과값 : ", model(input))
predict_result = model(input)
after_softmax = F.softmax(predict_result, dim = 1)
print("softmax 이후의 값", after_softmax)
print('-------------------------------------------------')

out = model(input) # 각 layer의 output 출력
print('-------------------------------------------------')

print(model.state_dict()) # 기존 model의 가중치 및 bias 출력
print('-------------------------------------------------')
print(model.state_dict()) # model model의 가중치 및 bias 출력
